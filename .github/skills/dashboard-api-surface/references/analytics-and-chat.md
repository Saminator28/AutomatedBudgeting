...
- Route owner: [src/ui/backend/routes/analytics.py](/src/ui/backend/routes/analytics.py)
- Supporting analysis modules:
  ...
  - [src/ai_analysis/outlier_detector.py](/src/ai_analysis/outlier_detector.py)
...
## Data Dependencies
- Most analytics endpoints query expenses and income through [src/ui/backend/deps.py](/src/ui/backend/deps.py), which returns DB-backed DataFrames.
- Budget endpoints depend on the expanded budget tables defined in [src/database/models.py](/src/database/models.py): `budget_goals`, `budget_settings`, `budget_history`, and `budget_goals_monthly`.
- Category rollups and investment detection depend on `config_categories` plus the keyword tables loaded into memory by [src/ui/backend/deps.py](/src/ui/backend/deps.py).

## Chatbot Pipeline
- Chat orchestrator: [src/ai_analysis/chatbot_assistant.py](/src/ai_analysis/chatbot_assistant.py)
- Model config source: [config/llm_models.json](/config/llm_models.json)
...
- Debug output: [logs/llm_prompt_debug.txt](/logs/llm_prompt_debug.txt)
...
- Main owner: [src/ui/src/InsightsPanel.js](/src/ui/src/InsightsPanel.js)
...
- Wrong math or aggregation: [src/ui/backend/routes/analytics.py](/src/ui/backend/routes/analytics.py), [src/ui/backend/deps.py](/src/ui/backend/deps.py)
- Bad advice or narrative framing: [src/ai_analysis/chatbot_assistant.py](/src/ai_analysis/chatbot_assistant.py), [src/ai_analysis/insights_generator.py](/src/ai_analysis/insights_generator.py), [src/ai_analysis/budget_advisor.py](/src/ai_analysis/budget_advisor.py)
- Model availability or Ollama transport issue: [docker-entrypoint.sh](/docker-entrypoint.sh), [src/ai_analysis/model_loader.py](/src/ai_analysis/model_loader.py), [config/llm_models.json](/config/llm_models.json)
- UI rendering or fetch flow issue: [src/ui/src/InsightsPanel.js](/src/ui/src/InsightsPanel.js), [src/ui/src/App.js](/src/ui/src/App.js)
