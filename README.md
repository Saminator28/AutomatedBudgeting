...
✅ **Clean merchant names** (removes extra numbers and locations)  
...
4. Extract to `C:\Program Files\poppler`
...
- ✅ Intelligent merchant name cleaning (spaCy NER + optional Ollama LLM)
...
- ✅ **Intelligent merchant name cleaning** - Two options:
  - **spaCy NER** (default, 13MB) - Fast named entity recognition for location/number removal
...
# Pull the uncensored dolphin-mistral model (recommended: 4GB)
ollama pull dolphin-mistral:latest
```

Ollama runs automatically as a background service after installation. If you see "address already in use" when running `ollama serve`, that means it's already running (which is what you want!).
...
**Why dolphin-mistral?**
- **Uncensored**: Won't refuse to process merchant names (censored models like llama3.2:1b reject financial data)
- **Smart**: Better at understanding context and cleaning complex merchant names
- **Fast**: Optimized Mistral 7B architecture runs efficiently on consumer hardware
- **Local**: 100% private, runs entirely on your machine

**Cleaning Options:**
- **Default (spaCy only):** Fast, accurate, works for 90% of cases without Ollama
- **Enhanced (Ollama + spaCy):** Smarter context-aware cleaning for edge cases

To enable Ollama cleaning, edit `process_monthly.py` line 383:
```python
statement_parser = StatementParser(clean_place_names=True, use_llm_cleaning=True)
```

**Alternative Models** (if dolphin-mistral doesn't work for you):
- `dolphin-llama3:8b` (4.7GB) - Uncensored Llama3, slightly more capable
- `phi3:mini` (2GB) - Microsoft's model, but may refuse some requests

**Important:** Standard models like llama3.2:1b have safety filters that incorrectly flag merchant names as harmful content. Always use an uncensored model for financial data processing.

The tool automatically falls back to spaCy if Ollama isn't installed or running.
